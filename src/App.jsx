...
export default function ExpenseTracker() {
  ...
  // ==============================================================
  // ផ្នែកទី១៖ រៀបចំ State ថ្មីសម្រាប់ផ្ទុកទិន្នន័យច្រើនជួរ (Multiple Rows)
  // ==============================================================
  const initialRow = {
    id: Date.now(), // ប្រើសម្រាប់ចំណាំជួរនីមួយៗ
    text: "",
    amount: "",
    type: "expense",
    category: "ទូទៅ",
  ...
  // មុខងារសម្រាប់បន្ថែមជួរថ្មី
  const addRow = () => {
    setFormRows([
      ...formRows,
      { ...initialRow, id: Date.now() + Math.random() },
    ]);
  };

  // មុខងារសម្រាប់លុបជួរណាមួយចោលវិញ (ពេលដូរចិត្ត)
  const removeRow = (idToRemove) => {
    if (formRows.length > 1) {
      setFormRows(formRows.filter((row) => row.id !== idToRemove));
    }
  };

  // មុខងារសម្រាប់អាប់ដេតទិន្នន័យពេលអ្នកប្រើប្រាស់វាយបញ្ចូលក្នុងជួរនីមួយៗ
  ...
  // ==============================================================
  // ផ្នែកទី២៖ មុខងារ Submit ដែលអាចបញ្ចូលទិន្នន័យច្រើនក្នុងពេលតែមួយ
  // ==============================================================
  const handleSubmit = async (e) => {
    e.preventDefault();

    // ឆែកមើលថាតើមានជួរណាដែលមិនទាន់បញ្ចូល ការពិពណ៌នា ឬទឹកប្រាក់ដែរឬទេ?
    const hasEmptyFields = formRows.some((row) => !row.text || !row.amount);
    if (hasEmptyFields)
      return alert("សូមបញ្ជូលការពិពណ៌នា និងចំនួនទឹកប្រាក់ឱ្យបានគ្រប់ជួរ!");

    try {
      if (editingId) {
        // បើកំពុងកែប្រែ (Edit) យើងធ្វើតែ ១ ជួរប៉ុណ្ណោះ
        const row = formRows[0];
        const numAmount = parseFloat(row.amount);
        const finalAmountUSD =
          row.currency === "KHR" ? numAmount / EXCHANGE_RATE : numAmount;
        const txDate = new Date(row.dateInput);
        ...
        // បើបញ្ចូលថ្មី យើងបញ្ចូលរាល់ទិន្នន័យទាំងអស់ដែលមានក្នុង formRows (អាចមានច្រើនជួរ)
        ...
        // រង់ចាំឱ្យការបញ្ចូលទិន្នន័យគ្រប់ជួរជោគជ័យទាំងអស់សិន
        await Promise.all(promises);
      }

      // បន្ទាប់ពីជោគជ័យ សម្អាតហ្វមចេញឱ្យសល់តែ ១ ជួរទទេស្អាតវិញ
      setFormRows([{ ...initialRow, id: Date.now() }]);
    } catch (error) {
  ...
  const handleEdit = (transaction) => {
    ...
    // ពេលចុច Edit យើងទាញទិន្នន័យនោះមកដាក់ក្នុងទម្រង់ formRows ជួរទី១
    setFormRows([
      {
        id: transaction.id,
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 font-sans print:bg-white print:p-0">
      <div className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-3xl print:shadow-none print:w-full print:max-w-none print:p-0">
        ...
        >
          ...
          {/* ============================================================== */}
          {/* ផ្នែកទី៣៖ បង្ហាញជួរនីមួយៗ ដោយប្រើ .map() លើ formRows */}
          {/* ============================================================== */}
          {formRows.map((row, index) => (
            <div
              key={row.id}
              className="space-y-3 mb-6 p-4 bg-white border border-slate-100 rounded-xl relative shadow-sm"
            >
              {/* បង្ហាញលេខរៀង និងប៊ូតុងលុប (មានតែបើជួរធំជាង១ និងមិនមែនកំពុង Edit) */}
              {formRows.length > 1 && !editingId && (
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100 px-2 py-1 rounded-lg"
                  >
                    ❌ លុបជួរនេះ
                  </button>
                </div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                  autoComplete="off"
                />
              </div>

              <div className="flex gap-2">
                <div className="flex w-1/2 bg-slate-50 border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 focus-within:bg-white">
                  <input
                    type="number"
                    step="any"
                  ...
                  <select
                    ...
                    className="bg-slate-100 border-l border-slate-200 px-2 font-bold text-slate-700 focus:outline-none cursor-pointer"
                  >
                ...
                <select
                  ...
                  className="w-1/2 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white cursor-pointer"
                >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  value={row.dateInput}
                  onChange={(e) =>
                    updateRow(row.id, "dateInput", e.target.value)
                  }
                  className="w-1/2 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-slate-700 cursor-pointer"
                />
                <select
                  value={row.type}
                  onChange={(e) => updateRow(row.id, "type", e.target.value)}
                  className="w-1/2 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white font-medium cursor-pointer"
                >
          ...
          {/* Datalist សម្រាប់ Autocomplete (ប្រើរួមគ្នាសម្រាប់គ្រប់ជួរ) */}
          ...
          {/* ប៊ូតុងបន្ថែមជួរ និងបញ្ជូនទិន្នន័យ */}
          <div className="flex gap-3 mt-4">
            {!editingId && (
              <button
                type="button"
                onClick={addRow}
                className="w-1/3 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 font-bold p-3 rounded-xl transition-all border border-indigo-100"
              >
                + បន្ថែមជួរ
              </button>
            )}
            <button
              type="submit"
              className={`${editingId ? "w-full" : "w-2/3"} text-white font-bold p-3 rounded-xl transition-all shadow-md ${editingId ? "bg-emerald-500 hover:bg-emerald-600" : "bg-indigo-600 hover:bg-indigo-700"}`}
            >
              {editingId
                ? "រក្សាទុកការកែប្រែ"
                : `បញ្ចូលទិន្នន័យ (${formRows.length})`}
            </button>
          </div>
        </form>
